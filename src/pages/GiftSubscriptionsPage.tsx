...
export function GiftSubscriptionsPage() {
  ...
  async function redeemGift(event: FormEvent) {
    ...
    setError(null)
    const { data, error: redeemError } = await invokeEdgeFunction<{ plan: string }>('redeem-gift', { body: { code } })
    if (redeemError) {
      setError(redeemError.message)
      return
    }
    if (!data?.plan) {
      setError('Gift redeemed but plan details were missing in response.')
      return
    }
