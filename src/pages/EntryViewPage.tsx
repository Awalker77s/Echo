...
export function EntryViewPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: entryData }, { data: ideasData }] = await Promise.all([
      ...
      if (entryData) {
        ...
        if (row.audio_url) {
          const { data: signed } = await supabase.storage.from('journal-audio').createSignedUrl(row.audio_url, 600)
  ...
  async function saveEdits(event: FormEvent) {
    ...
    const { data } = await supabase.from('journal_entries').update({ cleaned_entry: draft }).eq('id', entry.id).select('*').single()
    if (data) setEntry(data as JournalEntry)
  }

  if (!entry) return <div className="p-6">Loading entry...</div>
