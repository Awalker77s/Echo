...
export function InsightsPage() {
  ...
  useEffect(() => {
    async function loadPatterns() {
      if (!supabase) return
      const { data } = await supabase.from('patterns').select('*').eq('dismissed', false).order('confidence', { ascending: false })
      if (data) setPatterns(data as PatternInsight[])
      setLoading(false)
    }
  ...
  async function dismissPattern(id: string) {
    if (!supabase) return
    await supabase.from('patterns').update({ dismissed: true }).eq('id', id)
          ...
          return (
            <article key={pattern.id} className="app-card bg-gradient-to-br from-[#efeaff] to-[#fff7ef] p-5">
              ...
              <div className="mt-3 h-2 rounded-full bg-white/80">
                <div className="h-full rounded-full bg-[#7d74d2]" style={{ width: `${strength}%` }} />
              </div>
