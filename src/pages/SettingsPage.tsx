...
export function SettingsPage() {
  ...
  useEffect(() => {
    async function load() {
      if (!supabase) return
      const [{ data: profile }, { data: moodData }] = await Promise.all([
      ...
      if (moodData) setMoods(moodData as MoodPoint[])
    }
    void load()
  }, [])
  ...
  return (
    <main className="space-y-4">
      ...
      <section className="app-card space-y-4 p-5">
        ...
        <label className="flex items-center justify-between rounded-2xl bg-[#f8f2ec] p-3">
          <span>Weekly summary email</span>
          <input type="checkbox" checked={weeklyEmail} onChange={(event) => setWeeklyEmail(event.target.checked)} />
        </label>
        <label className="flex items-center justify-between rounded-2xl bg-[#f8f2ec] p-3">
          <span>Insight nudges</span>
          <input type="checkbox" checked={insightNudges} onChange={(event) => setInsightNudges(event.target.checked)} />
        </label>
      </section>

      <section className="app-card space-y-3 p-5">
        <h3 className="text-lg font-semibold">Data & sharing</h3>
        <button onClick={() => void exportJson()} disabled={busy} className="premium-button">
